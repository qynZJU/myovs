...
static inline int
fastnic_wc_init(struct flow_wildcards *wc, const struct flow *flow){
...
static inline int
fastnic_wc_parse(struct flow_wildcards *wc, const struct flow *flow){
...
static inline void
fastnic_off_flow_add(struct dp_netdev_pmd_thread *pmd,
                     struct dp_netdev_flow *flow, 
                     odp_port_t orig_in_port){
    struct match match;
    ...
    queue_netdev_flow_put(pmd, flow, &match, 
                          flow->actions.p->actions,
                          flow->actions.p->size, 
                          orig_in_port, 
                          DP_NETDEV_FLOW_OFFLOAD_OP_ADD);
}
    ...
    DP_PACKET_BATCH_REFILL_FOR_EACH (i, cnt, packet, packets_) {
        ...
        if (OVS_LIKELY(flow)) {
            ...
            if(pkt_seq >= OFFLOAD_THRE){
                fastnic_off_flow_add(pmd, flow, packet->md.orig_in_port);
            }
            #endif
            if (OVS_LIKELY(batch_enable)) {
                dp_netdev_queue_batches(packet, flow, tcp_flags, batches,
